...
class DubaiRtaScraper extends ScraperService
{
    private const BASE_URL   = 'https://www.rta.ae/wps/portal/rta/ae/about-rta/procurement/open-tenders';
    private const SOURCE_URL = 'https://www.rta.ae';
    ...
    protected function fetchListingPage(int $page): array
    {
        ...
        return [
            ...
            'has_more' => false, // RTA typically has single-page listing
        ];
    }
    ...
    private function parseListingHtml(string $html): array
    {
        ...
        $rows  = $xpath->query('//table//tbody/tr | //div[contains(@class,"tender")] | //li[contains(@class,"procurement")]');
        ...
        foreach ($rows as $row) {
            ...
            if ($cells->length >= 2) {
                ...
                $titleNode = $xpath->query('.//*[contains(@class,"title")]', $row);
            ...
            if (empty($refNumber)) {
                $refNumber = 'RTA-' . md5($title . $closingRaw);
            }

            $tenders[] = [
                'tender_number'     => $refNumber,
                'title'             => $title,
                'organization_name' => 'Roads and Transport Authority (RTA)',
