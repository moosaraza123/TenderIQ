...
class DubaiEsupplyScraper extends ScraperService
{
    private const BASE_URL   = 'https://www.dm.gov.ae/municipality-business/tenders-biddings/';
    private const SOURCE_URL = 'https://www.dm.gov.ae';

    public function getSourceSlug(): string
    {
        return 'dubai_esupply';
    }
    ...
    private function parseListingHtml(string $html): array
    {
        ...
        $rows  = $xpath->query('//table//tbody/tr | //li[contains(@class,"tender")] | //div[contains(@class,"tender")]');
        ...
        foreach ($rows as $row) {
            ...
            if ($cells->length >= 2) {
                ...
                $closingRaw = $cells->length > 3 ? trim($cells->item($cells->length - 1)?->textContent ?? '') : '';
            } else {
            ...
            if (empty($refNumber)) {
                $refNumber = 'DM-' . md5($title . $closingRaw);
            }

            $tenders[] = [
                'tender_number'     => $refNumber,
                'title'             => $title,
                'organization_name' => 'Dubai Municipality',
                'status'            => 'Published',
